...
export const fetchReportById = async (reportId: string, versionNumber?: string) => {
  try {
    ...
    if (versionNumber) {
      ...
      version =
        report.versions && report.versions.length > 0 ? [...report.versions].sort((a, b) => b.version_number - a.version_number)[0] : null;
    }
 ...
 */
export const createPreviewReport = async (projectId: string, zoneId: string, normId: string, datapointIds: string[] = []) => {
  try {
    // Load norm data
    ...
    for (const datapoint of datapointsToUse) {
      ...
      if (datapoint.ratings) {
        ...
        for (const [paramId, rating] of entries) {
          ...
          const { data: param, error } = await supabase.from("parameters").select("short_name, name").eq("id", paramId).single();
